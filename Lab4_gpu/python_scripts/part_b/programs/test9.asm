...
fn_main:
    addi sp, sp, -292
    mv   t0, s0              # hold caller's frame pointer
    addi s0, sp, 292          # s0 = frame pointer (caller's sp)
    sw   ra, -4(s0)
    sw   t0, -8(s0)          # saved caller's frame pointer

    sw   zero, -268(s0)          # init x = 0
    sw   zero, -272(s0)          # init a = 0
    sw   zero, -276(s0)          # init b = 0
    sw   zero, -280(s0)          # init sum = 0
    sw   zero, -284(s0)          # init count = 0
    sw   zero, -288(s0)          # init m = 0
    sw   zero, -292(s0)          # init i = 0
    li   t0, 0
    push t0
    pop  t0
    sw   t0, -280(s0)          # assign sum
    push t0
    pop  t0
    li   t0, 0
    push t0
    pop  t0
    sw   t0, -284(s0)          # assign count
    push t0
    pop  t0
__L_while_top_3:
    li   t0, 1
    push t0
    pop  t0
    beqz t0, __L_while_end_4
    addi sp, sp, -4          # scanf: matched-count slot
    sw   zero, 0(sp)
    call __read_int          # %d
    blez a1, __L_scan_5_fail
    push a0
    addi t0, s0, -268          # &x
    push t0
    pop  t1
...
__L_while_top_8:
    ...
    push t0
    pop  t0
    beqz t0, __L_while_end_9
    addi sp, sp, -4          # scanf: matched-count slot
    sw   zero, 0(sp)
    call __read_int          # %d
    blez a1, __L_scan_10_fail
    push a0
    addi t0, s0, -272          # &a
    push t0
    pop  t1
...
__L_while_top_14:
    ...
    push t0
    pop  t0
    beqz t0, __L_while_end_15
    addi sp, sp, -4          # scanf: matched-count slot
    sw   zero, 0(sp)
    call __read_int          # %d
    blez a1, __L_scan_16_fail
    push a0
    addi t0, s0, -268          # &x
    push t0
    pop  t1
...
__L_while_top_19:
    ...
    push t0
    pop  t0
    beqz t0, __L_while_end_20
    addi sp, sp, -4          # scanf: matched-count slot
    sw   zero, 0(sp)
    call __read_int          # %d
    blez a1, __L_scan_21_fail
    push a0
    addi t0, s0, -268          # &x
    push t0
    pop  t1
