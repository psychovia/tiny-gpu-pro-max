...
fn_main:
    ...
    addi s0, sp, 288          # s0 = frame pointer (caller's sp)
    sw   ra, -4(s0)
    sw   t0, -8(s0)          # saved caller's frame pointer

    sw   zero, -268(s0)          # init n = 0
    sw   zero, -272(s0)          # init i = 0
    sw   zero, -276(s0)          # init x = 0
    sw   zero, -280(s0)          # init sum = 0
    sw   zero, -284(s0)          # init mn = 0
    sw   zero, -288(s0)          # init mx = 0
    li   t0, 0
    push t0
    pop  t0
    sw   t0, -268(s0)          # assign n
    push t0
    pop  t0
__L_while_top_1:
    li   t0, 1
...
__L_while_end_2:
    ...
    push t0
    pop  t0
    sw   t0, -288(s0)          # assign mx
    push t0
    pop  t0
    li   t0, 0
    push t0
    pop  t0
    sw   t0, -272(s0)          # assign i
    push t0
    pop  t0
__L_while_top_6:
    lw   t0, -272(s0)          # read i
