...
// Register decode over two `fifo`s. The Pi pushes/pops each FIFO over I2C:
//   0x00 STATUS  bit0 rx_empty bit1 rx_full bit2 tx_empty bit3 tx_full bit4 overflow
//   0x01 RX  write=push  read=pop      0x02 TX  write=push  read=pop
...
module io_bridge (
    ...
    logic rx_push, rx_pop, tx_push, tx_pop;
    assign rx_push = ready   & ~expect_ptr & (reg_ptr == REG_RX);
    assign tx_push = ready   & ~expect_ptr & (reg_ptr == REG_TX);
    assign rx_pop  = rd_load & (reg_ptr == REG_RX);
    assign tx_pop  = rd_load & (reg_ptr == REG_TX);

    logic       rx_full, rx_empty, rx_overflow;
    logic [7:0] rx_rdata, rx_count;
    logic       tx_full, tx_empty, tx_overflow;
    logic [7:0] tx_rdata, tx_count;

    fifo rx_fifo (.clk, .rst, .push(rx_push), .wdata(data), .full(rx_full),
                  .pop(rx_pop), .rdata(rx_rdata), .empty(rx_empty),
                  .count(rx_count), .overflow(rx_overflow));

    fifo tx_fifo (.clk, .rst, .push(tx_push), .wdata(data), .full(tx_full),
                  .pop(tx_pop), .rdata(tx_rdata), .empty(tx_empty),
    ...
    always_ff @(posedge clk) begin : last_byte_tracker
        if(rx_push | tx_push) last_byte <= data;
    end
